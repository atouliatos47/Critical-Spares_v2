...
const Components = {
    ...
    showBarcodeModal(id) {
        ...
        modal.innerHTML = `
            <h3>Barcode Label</h3>
            <div id="barcodeLabel" style="background:white; border:1px solid #e2e5ea; border-radius:8px; padding:16px; margin:16px 0; text-align:center; font-family:'DM Sans',sans-serif;">
                <div style="font-size:11px; font-weight:700; color:#2D4A5C; text-transform:uppercase; letter-spacing:1px; margin-bottom:4px;">Clamason Industries</div>
                <div style="font-size:15px; font-weight:700; color:#1a1a2e; margin-bottom:2px;">${Utils.escapeHtml(item.name)}</div>
                ${item.location ? '<div style="font-size:11px; color:#6b7280; margin-bottom:2px;">📍 ' + Utils.escapeHtml(item.location) + '</div>' : ''}
                ${wsName ? '<div style="font-size:11px; color:#6b7280; margin-bottom:8px;">🏭 ' + Utils.escapeHtml(wsName) + '</div>' : '<div style="margin-bottom:8px;"></div>'}
                <svg id="barcodesvg"></svg>
                <div style="font-size:10px; color:#6b7280; margin-top:4px; letter-spacing:1px;">${Utils.escapeHtml(barcodeValue)}</div>
            </div>
            <div class="modal-actions">
                <button class="modal-btn cancel" onclick="Components.closeModal()">Close</button>
