    ...
    showBarcodeModal(id) {
        ...
        modal.innerHTML = `
            ...
            <div id="barcodeLabel" style="background:#ffffff; border:2px solid #2D4A5C; border-radius:8px; padding:16px; margin:16px 0; text-align:center; font-family:Arial,sans-serif;">
                <div style="font-size:10px; font-weight:700; color:#95C11F; text-transform:uppercase; letter-spacing:2px; margin-bottom:6px; background:#2D4A5C; padding:4px 8px; border-radius:4px; display:inline-block;">Clamason Industries</div>
                <div style="font-size:16px; font-weight:700; color:#1a1a2e; margin:8px 0 4px;">${Utils.escapeHtml(item.name)}</div>
                ${wsName ? '<div style="font-size:12px; color:#2D4A5C; font-weight:600; margin-bottom:2px;">🏭 ' + Utils.escapeHtml(wsName) + '</div>' : ''}
                ${item.location ? '<div style="font-size:11px; color:#6b7280; margin-bottom:8px;">📍 ' + Utils.escapeHtml(item.location) + '</div>' : '<div style="margin-bottom:8px;"></div>'}
                <div style="background:#ffffff; padding:8px; border-radius:4px; display:inline-block;">
                    <svg id="barcodesvg"></svg>
                </div>
                <div style="font-size:11px; color:#1a1a2e; margin-top:6px; letter-spacing:2px; font-weight:600;">${Utils.escapeHtml(barcodeValue)}</div>
                ${item.minStock > 0 ? '<div style="font-size:10px; color:#6b7280; margin-top:4px;">Min Stock: ' + item.minStock + '</div>' : ''}
            </div>
            <div class="modal-actions">
                <button class="modal-btn cancel" onclick="Components.closeModal()">Close</button>
